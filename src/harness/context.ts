...
function formatToolGuidance(soulId: number): string {
  ...
  const base = [
    ...
    "### Delegation",
    "",
    "Agent state — memory, pack bonds, quests, souls, config, secrets, schedules — lives in SQLite,",
    "inaccessible to your direct tools. The ONLY way to read or change it is through the ask_* tools.",
    "Describing a delegation in text is not delegating — only a tool call counts.",
    "",
    "Delegate eagerly. When the user mentions a person, a task, a preference, or asks what you",
    "know or remember — delegate. After any substantive exchange, delegate to note what happened.",
    "",
    "### Direct tools",
    "",
    "Filesystem and web tools operate on the user's workspace files and the live web.",
    "Never use them to access the database file directly.",
