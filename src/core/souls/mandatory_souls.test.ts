...
describe("MANDATORY_SOUL_IDS", () => {
  ...
  it("does not include js-engineer", () => {
    ok(!("js-engineer" in MANDATORY_SOUL_IDS));
  });
});
...
describe("MANDATORY_SOUL_NAMES", () => {
  ...
  it("is a subset of DEFAULT_SOULS keys", () => {
    const defaultKeys = new Set(Object.keys(DEFAULT_SOULS));
    for (const name of MANDATORY_SOUL_NAMES) {
      ok(defaultKeys.has(name), `Mandatory soul ${name} missing from DEFAULT_SOULS`);
    }
  });
});
