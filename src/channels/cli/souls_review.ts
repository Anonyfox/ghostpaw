...
import { buildReviewPrompt, createEntity, invokeMentor } from "../../harness/index.ts";
import { style } from "../../lib/terminal/index.ts";
import { withRunDb } from "./with_run_db.ts";

export default defineCommand({
  meta: { name: "review", description: "Ask the mentor to review a soul" },
  args: {
    name: {
      type: "positional",
  ...
  async run({ args }) {
    ...
    await withRunDb(async (db) => {
      ...
      console.log(style.dim(`Reviewing "${soul.name}" via mentor...`));
