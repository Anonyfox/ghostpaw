...
import type { StorylineStatus, UpdateStorylineInput } from "../../core/quests/api/types.ts";
...
export default defineCommand({
  meta: { name: "storyline-update", description: "Modify a storyline" },
  args: {
    id: {
      type: "positional",
  ...
  async run({ args }) {
    ...
    try {
      await withRunDb((db) => {
        ...
        const log = updateStoryline(db, id, input);
        console.log(
