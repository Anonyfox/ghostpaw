...
export default defineCommand({
  meta: { name: "storyline-show", description: "Show storyline details and its quests" },
  args: {
    id: {
      type: "positional",
  ...
  async run({ args }) {
    ...
    await withRunDb((db) => {
      const log = getStoryline(db, id);
      if (!log) {
        errorLine(`Storyline #${id} not found.`);
        return;
      }

      console.log(style.cyan(`Storyline #${log.id} — ${log.title}`));
      console.log();
      ...
      f(
        "breakdown",
        `${progress.accepted} accepted / ${progress.active} active / ${progress.blocked} blocked / ${progress.done} done`,
      );
      ...
      const quests = listQuests(db, { storylineId: log.id, limit: 100 });
      if (quests.length > 0) {
        console.log();
