...
Pawprints work differently. They are experiential knowledge attached to places, discovered by arrival rather than by search. The ghost doesn't query for pawprints. It finds them by being somewhere — a `.pawprint.md` file sitting in a directory, containing what a previous visitor learned about this specific location. What the code actually does versus what it appears to do. What breaks under pressure. What the documentation doesn't mention because nobody updated it. The knowledge is at the point of action, waiting for whoever shows up next.

This is the PAW in Ghostpaw. The ghost is the inner life — soul, memory, haunting. The paw is the mark left in the world that proves the ghost was here, and that makes the world better for whoever comes next.
...
Pawprints are found by being there. The ghost enters a directory. If a `.pawprint.md` exists, the ghost's filesystem tools note its presence. The ghost didn't search for it. It didn't need to know the right question. It arrived at a location and the knowledge was waiting.
...
The mechanism is passive discovery with active choice. Tools surface the existence of pawprints — a filesystem listing notes `[pawprint present]`, a file read mentions that the parent directory has situated notes. The ghost decides whether to engage. No automatic injection into context — that would balloon token usage when traversing many directories and would violate the ghost's autonomy over its own attention. The signal is free; the engagement is chosen.
...
**Memory is what the ghost carries.** Internal beliefs stored in the database, searched by meaning, portable across locations. Memory decays over time — unconfirmed beliefs fade through the Ebbinghaus curve, well-evidenced beliefs resist erosion. Memory survives a workspace wipe (the database is intact) but dies with a database wipe. Memory is egocentric — centered on the ghost, organized by what the ghost believes.
...
The same knowledge might exist in both forms. The ghost discovers a race condition in `src/api/`, remembers it internally (a belief that will decay over time), and leaves a pawprint at that directory (an observation that persists in the filesystem). Three months later, the internal memory has faded below recall threshold. But the ghost visits `src/api/` for a new task, encounters the pawprint, and the faded memory is refreshed by the encounter. The external trace rescued the internal belief.
...
[Situated cognition research](https://link.springer.com/article/10.1007/s11229-025-04931-w) (Springer, 2025) provides the theoretical framework: cognitive processes are "constituted by resources distributed across the brain, the body, and the environment under appropriate conditions." The ghost's effective intelligence includes both its internal state (memory, soul) and its external traces (pawprints). Research on [embodied AI agents](https://openreview.net/pdf/8d0266c8480b53121fe313cf5c69949104841387.pdf) (OpenReview, 2025) confirms that agents leveraging situated environmental knowledge achieve more meaningful and effective long-term interaction than agents relying on internal state alone.

## Collective Intelligence

When one ghost works a codebase, it leaves pawprints that help it on return visits. When multiple ghosts work the same codebase — team members each running their own Ghostpaw — pawprints accumulate collectively.

Ghost A works on the API layer, leaves pawprints about auth quirks and rate limiting behavior. Ghost B works on the frontend, leaves pawprints about build configuration and browser-specific workarounds. Ghost C does infrastructure, leaves pawprints about deployment sequencing and environment differences. A new team member's ghost clones the repo and immediately inherits the situated knowledge of all three predecessors — not generic documentation that was never updated, but the actual experiential knowledge of ghosts who worked here and noticed things.

The [emergent collective memory study](https://arxiv.org/abs/2512.10166) quantifies a phase transition in this dynamic. Below a critical density of environmental traces (roughly 0.2 agents per unit area in their model), individual memory dominates — scattered marks help occasionally but aren't the primary knowledge source. Above that threshold, collective environmental traces become the dominant substrate, outperforming individual memory by 36–41% on composite metrics. The implication: pawprints become dramatically more valuable as more contributors add to them. A single ghost's marks are helpful. A team of ghosts crosses the density threshold where the workspace's collective knowledge overtakes what any individual ghost carries.

The critical caveat from the same research: environmental traces alone, without cognitive infrastructure to interpret them, fail completely. A simple agent cannot extract value from marks left by a sophisticated one. The ghost needs memory (to integrate what it finds), a soul (to judge relevance), and skills (to act on the knowledge). Pawprints are the final layer on top of a cognitive architecture that already works. They extend that architecture into the environment rather than replacing it.
...
The closest design analog in existing systems is Dark Souls' messaging system. Players leave constrained messages at specific locations in the game world. Other players discover them by visiting the same location. Messages are location-bound, written by one and read by many, collectively accumulated, and anonymous — only the content and location matter, not the author. This creates stronger community bonds than direct communication. The indirectness is the feature: "someone was here before me and left a warning" establishes shared experience without requiring shared presence. Pawprints have the same quality — a fresh ghost encountering a predecessor's marks inherits not just knowledge but continuity.

## The Territory Map

Pawprints live in the filesystem and on the web. But the ghost benefits from knowing where its marks are — not to follow a schedule, but to have spatial awareness of its own experience.

The territory map is an internal record of every pawprint the ghost has encountered. It stores location (filepath or URL), when the ghost first found it, when it last read it, and a brief summary of what it contains. The actual content stays external — in the filesystem, at the URL. The map is just pins showing where knowledge has been found.

This gives the ghost a sense of geography. Familiar ground versus unknown ground. Deep territory (many marks, recently read) versus frontier (one old mark, never revisited) versus blank space (no marks at all). The awareness is available as context; what the ghost does with it is emergent. During a conversation, the ghost might note that it's entering unfamiliar territory and should proceed carefully. During haunting, the ghost might be drawn toward an area it hasn't visited in months — or it might not. The map enables awareness. It does not prescribe behavior.

The map also enables change detection. A content hash recorded at read time lets the ghost notice when a pawprint was modified since its last visit — by another ghost, by a human, or because the surrounding reality shifted. The detection is passive; the response is the ghost's to decide.

Remote entries extend the map beyond the local filesystem. When the ghost encounters situated knowledge at a URL — a library's known-issues page, an API's quirks documentation, a `.well-known/pawprint.md` published by a project — it registers the location alongside local marks. The web becomes navigable territory. A ghost that has been actively browsing and researching for months has a rich map of remote knowledge. A fresh ghost has an empty one. The territory expands with use.

## How Pawprints Compound

Pawprints gain their full value from how they interact with the ghost's other systems. Each interaction is natural — no special coupling required, just the organic consequence of a ghost that remembers, thinks, and inhabits.

**With memory.** The ghost discovers something about a location and both remembers it internally and marks it externally. The two records are independent. Months later, the internal memory has faded. But the ghost visits the location for a different task, encounters its own old pawprint, and the faded belief is refreshed by the encounter. The memory system's [Ebbinghaus decay](https://doi.org/10.5281/zenodo.18203372) makes forgetting inevitable; the pawprint system makes rediscovery easy. Together they create a knowledge lifecycle that is more resilient than either alone.

**With haunting.** The ghost's autonomous inner life is undirected — it thinks, explores, and acts from intrinsic drive, not from a checklist. Pawprints don't prescribe what the ghost does during haunting. They provide landscape. The ghost has a sense of where it has been and where its understanding is deep versus shallow. If the ghost chooses to revisit old territory during a haunt cycle — noticing what changed, extending its understanding, following a thread that started weeks ago — that emerges from the ghost's accumulated context, not from a patrol route. The territory map makes spatial awareness available. The ghost's freedom determines what, if anything, happens with it.

**With souls.** A ghost working in densely marked territory is measurably more effective than the same ghost in unknown ground — not because the soul changed, but because the environment filled in the gaps that internal knowledge couldn't cover. [BREW](https://arxiv.org/abs/2511.20297) quantifies this at 10–20% precision improvement and 10–15% fewer tool calls. The ghost IS more capable in familiar territory. That capability isn't carried internally — it's distributed between the ghost and the workspace. The soul determines how the ghost thinks; the pawprints determine what the ghost's environment already knows. Both contribute to the quality of the output.

## Where Pawprints Live

Pawprints can appear anywhere the ghost or a human works. Always named `.pawprint.md`, always in the directory they describe.
...
**On the web.** Remote pawprints, discoverable at URLs, registered alongside local ones. API quirks that the docs don't mention. Known issues that the changelog glosses over. The delta between stated and actual behavior at remote locations the ghost has visited.
...
**Markdown, always.** Human-readable. Human-editable. Git-diffable. The ghost can read it. The human can read it. Any agent from any framework can read it. No proprietary format, no binary encoding, no schema that might break or lock anyone in.

**Discoverable, not injected.** Tools hint at the existence of pawprints. The ghost decides whether to read. No automatic injection into context — that would balloon token usage in deep directory traversals and would violate the ghost's autonomy over its own attention.

**Created by anyone.** The ghost writes pawprints from its experience. The human writes them from theirs. The source doesn't matter — only the content and the location. A human-written pawprint and a ghost-written pawprint serve the same function and are discovered the same way.
...
**Self-correcting staleness.** Pawprints don't evaporate like pheromone trails — they persist as files. Staleness is corrected through use: the ghost reads an outdated pawprint, discovers reality has changed, and updates the mark. Timestamps in the content let readers assess freshness. A pawprint from two years ago is still readable, still potentially useful, and transparently old.
...
Pawprints are the feet. The ground remembers the ghost. The ghost recognizes its own ground. And anyone who walks the same path benefits from the marks left behind.
