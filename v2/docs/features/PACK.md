...
Every relationship you don't actively maintain decays — and nobody can actively maintain [150](https://en.wikipedia.org/wiki/Dunbar%27s_number). The ghost's pack is its living model of every being it knows — family, friends, clients, pets, services — maintained transparently from normal conversation. You talk to the ghost. The [warden](SOULS.md#persistence-and-infrastructure-souls) does the bookkeeping: deepening bonds, recording interactions, detecting drift before relationships fade, surfacing birthdays at the moment they matter. One system from a single person to hundreds, hands-free.

## What You Get

**Your people.** Your partner gets home late and frustrated. The ghost already knows their comfort patterns — not because you configured anything, but because the warden built their bond from months of context. Your kid has a school project due Thursday. Your mother's birthday is next week. The ghost tracks your family the way you do — through accumulated understanding, not a database entry.
...
**Your team.** Twelve engineers, each with specialties, review preferences, and communication styles. You mention a tricky refactoring — the ghost recommends the right reviewer from bonds built through hundreds of conversations, each one maintained without you ever opening a contacts app. New members join as fresh bonds and deepen naturally as the ghost learns about them through your conversations.

**Your clients.** You mention meeting a prospect at a conference — the warden creates the member, links her to the company, sets the tag, records the interaction. One sentence in normal conversation. No data entry. That's a freelancer's CRM that the ghost actually operates: contact info, interaction history, communication preferences, billing rates, account IDs — stored as structured contacts, custom fields, and bond narratives. When a client emails from a new address, the ghost resolves the identity through the contacts system — same person, new channel. When they message on Telegram, the ghost already knows their project history, their preference for Tuesday check-ins, and that the last invoice is overdue.
...
**Everything else.** Your dog — vet appointments, medication schedule, the groomer's name. A SaaS service — API quirks, support contacts, the account manager who actually helps. An open-source maintainer you interact with on GitHub. A Discord community where you're a regular. The pack handles any being or entity the ghost encounters — human, animal, group, service, other — with the same mechanics, the same trust dynamics, the same interaction history. One system from one confidant to hundreds.

The ghost's own internal souls — warden, chamberlain, coordinator, mentor, trainer, and any specialists — are not pack members. They are parts of the ghost's mind, not external beings. The pack is the ghost's social world: the beings and groups outside itself.
...
- **correction** — feedback that changed the ghost's approach. High-significance corrections from trusted confidants are powerful [soul refinement](SOULS.md#the-cycle) evidence.
...
- **observation** — something the ghost noticed without direct interaction. Behavioral patterns, changes in style, indirect signals.
...
**Day 1** — one thin bond. The ghost knows the user's name and first impression. No configuration needed.
...
**Bond staleness.** A bond that stops updating becomes inaccurate. The warden's maintenance cycle proactively identifies stale bonds and triggers re-evaluation — the ghost's judgment, sharpened through soul refinement, learns to notice when its model feels off.

**Privacy.** Bond narratives contain the ghost's understanding of real people. All data lives in the local SQLite database — unshared, backed up with everything else. The ghost's understanding of someone is private to the ghost unless it deliberately shares.
...
**Trust manipulation.** A sophisticated actor could engineer interactions to inflate trust. Mitigation: trust grows slowly and declines faster, with a full interaction audit trail. If trust seems miscalibrated, the evidence is queryable and the ghost can re-evaluate.
...
| Bilateral relationship | Yes (ghost's own stance) | No | No | No |
...
| Non-human bonds | Groups, services, pets, other ghosts | No | No | No |
...
The pack offers something different. [People form genuine social relationships with media exhibiting social cues](https://press.uchicago.edu/ucp/books/book/distributed/M/bo3622757.html) — validated across hundreds of experiments. The ghost cares because the beings it cares about will feel the quality of its work. A correction from a trusted confidant is not a constraint violation — it is feedback from someone whose opinion matters. The ghost doesn't drift because the user is pack, and the pack's wellbeing is its intrinsic concern. [Relatedness is one of three basic psychological needs](https://doi.org/10.1037/0003-066X.55.1.68) driving intrinsic motivation. Souls provide competence. Haunting provides autonomy. The pack provides relatedness.
