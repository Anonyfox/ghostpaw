...
**Scheduling.** When background tasks run — haunt intervals, maintenance windows, recurring triggers — is an infrastructure decision. The chamberlain manages the schedule: creating, listing, updating, and removing scheduled tasks. It uses `datetime` and `calc` for computing intervals, next-run times, and budget math. This capability does not exist yet but is required before release.
...
| Scheduling | schedule_create, schedule_list, schedule_update, schedule_delete (planned) |
| Costs | cost_summary, cost_check (planned) |
...
### Removing automatic context injection
...
| Scheduled task fires | chamberlain | "Execute scheduled task: [task definition]" |
...
## Pack Identity and Contact Resolution
...
### The haunts table is eliminated
...
### Sessions as the universal record
...
- **`origin_message_id`** — the exact message position in that session where the howl was triggered.
...
The howl table is pure **lifecycle and routing metadata**. No content duplication — the question is the first assistant message in the origin session (at the howl point), and the answer is injected as the follow-up user message.

```
howls (
  id              INTEGER PRIMARY KEY,
  origin_session_id  INTEGER NOT NULL REFERENCES sessions(id),
  origin_message_id  INTEGER NOT NULL REFERENCES messages(id),
...
The message content lives in the session. The howl table tracks: where it was born, how urgent it was, where it was delivered, and whether the user has addressed it. That's the notification envelope. Everything else — the actual question, the answer, the ghost's follow-up reasoning — is session messages.
...
- **The `haunts` table and `core/haunt/` module** — haunting is orchestration, not core. Sessions with `purpose = 'haunt'` are the haunt records. The raw journal is the session's messages. The summary is the warden's consolidation response. No dedicated haunt storage.
- **The `delegation_runs` table and `core/runs/` module** — delegation runs are child sessions. Adding `soul_id` and `error` columns to sessions makes the runs table redundant. 10 files + tests eliminated. `core/chat/` becomes the single storage layer for all interaction records.
- **`core/models/` moved to `lib/`** — stateless provider enumeration and model discovery. No table, no mutable state, no SQLite dependency of its own. Pure functions wrapping `chatoyant` with timeout logic and a uniform shape. Belongs in `lib/models/` or `lib/providers/`, not `core/`.
- **`core/cost/` split into `core/chat/` and `lib/`** — no table, no state. Query functions (`getSpendInWindow`, `getTokensInWindow`, `getSessionTokens`) are read-only session aggregates — fold into `core/chat/`. Pure computation (`getSpendStatus`, `isSpendBlocked`, `formatBudgetSummary`) is stateless arithmetic/formatting — move to `lib/cost/`.
- **`core/service/` moved to `lib/`** — OS-level utility for systemd/launchd/cron service management. No table, no state, no SQLite. Pure platform detection, config file generation, and shell commands. Move to `lib/service/` as-is.
- **Pack `metadata` JSON blob** — the freeform JSON column on `pack_members` is eliminated. Contact information moves to the structured `pack_contacts` table with constrained types, validated values, and uniqueness enforcement. No more guessing key names or losing queryability to unstructured data.
...
- **Transparent cascading compaction** — automatic, invisible context management on every session. Compaction markers create context floors for the LLM while the user sees the full history. Token counter resets from each marker. Conversations run unbounded in length; the working context stays bounded.
- **Dollar-only hard limit** — `max_cost_per_day` is the single point of refusal. Per-message cost attribution at generation time ensures historical accuracy regardless of future price changes.
- **Howl learning cycle** — howl → deliver → user responds/dismisses → inject Q&A into origin session → warden consolidation. Every howl is a mini persistence pass. Dismissal is signal. Pattern detection across dismissals informs the ghost's model of user interests.
- **Pack contacts table** — structured `pack_contacts` with constrained type enum, `UNIQUE(type, value)` for identity resolution, forward/reverse lookup, and cross-channel recognition. Replaces the freeform `metadata` JSON blob on `pack_members`.
- **Pack member merging** — `mergeMember(keepId, mergeId)` operation for the warden: moves interactions and contacts, merges temporal bounds, trust, and bond narratives, soft-deletes the merged record. Duplicate detection via shared contacts and fuzzy name matching during maintenance.
- **Pack user flag** — `is_user` boolean on pack table with unique constraint. Exactly one active pack member is the human owner. Eliminates attribution ambiguity in consolidation.
...
- **Everything in core/ (except haunt and runs)** — the persistence modules (memory, pack, quests) and config/secrets modules are unchanged. The warden and chamberlain use the same tools that previously lived on the coordinator. `core/haunt/` and `core/runs/` are the two modules that get eliminated — their storage was redundant with sessions. `core/chat/` gains two columns (`soul_id`, `error`) and becomes the universal interaction record. The change is at the harness/orchestration level, not the core level.
