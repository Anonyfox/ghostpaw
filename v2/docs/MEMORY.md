...
Eight knobs control memory behavior at runtime. All are tunable through the agent's config tools, the CLI, or the web UI — no restart required. See [Configuration](features/SETTINGS.md#configuration) for the full configuration system. The agent can adjust these during reflection to tune its own effectiveness based on observed behavior.
