...
describe("createRequest", () => {
  it("builds a valid JSON-RPC request with incrementing ids", () => {
    const a = createRequest("initialize", { protocolVersion: "2025-03-26" });
    strictEqual(a.id, 1);
    strictEqual(a.msg.jsonrpc, "2.0");
    strictEqual(a.msg.method, "initialize");
    deepStrictEqual(a.msg.params, { protocolVersion: "2025-03-26" });
...
describe("createNotification", () => {
  ...
  it("includes params when provided", () => {
    const n = createNotification("test", { key: "value" });
    deepStrictEqual(n.params, { key: "value" });
  });
});
...
describe("isJsonRpcMessage", () => {
  ...
  it("handles stderr noise lines", () => {
    ...
    ok(!isJsonRpcMessage("Debugger attached."));
  });
});
...
describe("parseResponse", () => {
  ...
  it("throws on non-object", () => {
    ...
    throws(() => parseResponse("null"), /must be an object/);
    throws(() => parseResponse("42"), /must be an object/);
  });
...
describe("isErrorResponse", () => {
  ...
  it("identifies success responses", () => {
    const success = parseResponse('{"jsonrpc":"2.0","id":1,"result":"done"}');
    ok(!isErrorResponse(success));
  });
});
