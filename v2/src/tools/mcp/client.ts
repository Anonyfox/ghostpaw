import { createNotification, createRequest, isErrorResponse } from "./jsonrpc.ts";
import type {
...
export async function connectMcpServer(
  ...
  const result = await withTimeout(
    (async () => {
      ...
      if (!initResp) {
        throw new Error(`MCP server "${serverName}" returned no initialize response`);
      }
      ...
      if (!listResp) {
        throw new Error(`MCP server "${serverName}" returned no tools/list response`);
      }
  ...
  async function callTool(
    ...
    if (!resp) {
      throw new Error(`MCP server "${serverName}" returned no response for tools/call`);
    }
