...
export function updateHowlStatus(db: DatabaseHandle, id: number, status: HowlStatus): void {
  const now = status === "responded" || status === "dismissed" ? Date.now() : null;
  db.prepare(
    "UPDATE howls SET status = ?, responded_at = COALESCE(?, responded_at) WHERE id = ?",
  ).run(status, now, id);
}
