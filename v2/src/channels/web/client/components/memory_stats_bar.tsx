...
export function MemoryStatsBar({ stats, onShowStale }: MemoryStatsBarProps) {
  ...
  return (
    <div class="mb-3 small">
      ...
      <div class="d-flex flex-wrap gap-3 mt-1 text-body-tertiary" style="font-size: 0.8em">
        {Object.entries(stats.bySource).map(([source, count]) => (
          <span key={source}>
            {source} {count}
          </span>
        ))}
        {stats.singleEvidence > 0 && (
          <span title="Memories with only one evidence point — never confirmed">
            {stats.singleEvidence} unconfirmed
          </span>
        )}
        {stats.recentRevisions > 0 && (
          <span title="Memories revised in the last 30 days">
            {stats.recentRevisions} revised recently
          </span>
        )}
      </div>
    </div>
