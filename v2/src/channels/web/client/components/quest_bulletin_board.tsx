...
export function QuestBulletinBoard({ onQuestClick, boardCount }: Props) {
  ...
  const total =
    ctx.overdue.length + ctx.pendingReminders.length +
    ctx.todayEvents.length + ctx.activeQuests.length + ctx.dueSoon.length;

  if (total === 0 && (!boardCount || boardCount === 0)) {
    return (
      <div class="text-body-tertiary small mb-3">
        All clear — no urgent quests.
      </div>
    );
  }
  ...
  if (ctx.pendingReminders.length > 0) counts.push(`${ctx.pendingReminders.length} reminder${ctx.pendingReminders.length !== 1 ? "s" : ""}`);
    ...
    <div class="mb-3">
        ...
        <div class="bulletin-section border-start border-3 border-danger ps-3 py-1 mb-2">
          ...
          {ctx.overdue.map((q) => (
            <div key={q.id} class="small">
              <a href="#" class="text-body text-decoration-none" onClick={click(q.id)}>
                {q.title}
              </a>
      ...
      {boardCount != null && boardCount > 0 && (
        <div class="small text-warning mb-2">
          <span class="fw-semibold">!</span> {boardCount} quest{boardCount !== 1 ? "s" : ""} on the board
        </div>
      )}
    </div>
  );
