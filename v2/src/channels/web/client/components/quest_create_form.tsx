  ...
  return (
    <div class="border rounded p-3 mb-3 bg-body-tertiary">
      ...
      <div class="row g-2 mb-2">
        <div class="col-12">
          <input type="text" class="form-control form-control-sm" placeholder="Quest title *"
        ...
        <div class="col-sm-4">
          <select class="form-select form-select-sm" value={form.priority ?? "normal"}
        ...
        <div class="col-sm-4">
          <select class="form-select form-select-sm" value={String(form.questLogId ?? "")}
            onChange={(e) => { const v = (e.target as HTMLSelectElement).value; setForm({ ...form, questLogId: v ? Number(v) : undefined }); }}>
            <option value="">No quest log</option>
            {logs.map((l) => <option key={l.id} value={String(l.id)}>{l.title}</option>)}
          </select>
        </div>
        <div class="col-sm-4">
          <input type="text" class="form-control form-control-sm" placeholder="Tags"
            value={form.tags ?? ""} onInput={(e) => setForm({ ...form, tags: (e.target as HTMLInputElement).value || undefined })} />
        </div>
        <div class="col-sm-6">
        ...
        <div class="col-sm-6">
          ...
          <input type="datetime-local" class="form-control form-control-sm"
            onInput={(e) => setForm({ ...form, remindAt: fromLocal((e.target as HTMLInputElement).value) })} />
        </div>
        <div class="col-sm-6">
          <label class="form-label small text-body-secondary mb-0">Starts</label>
          <input type="datetime-local" class="form-control form-control-sm"
            onInput={(e) => setForm({ ...form, startsAt: fromLocal((e.target as HTMLInputElement).value) })} />
        </div>
        <div class="col-sm-6">
          <label class="form-label small text-body-secondary mb-0">Ends</label>
          <input type="datetime-local" class="form-control form-control-sm"
            onInput={(e) => setForm({ ...form, endsAt: fromLocal((e.target as HTMLInputElement).value) })} />
        </div>
        <div class="col-12">
