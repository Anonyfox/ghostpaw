...
import { getSoul, listDeletedSouls, restoreSoul } from "../../core/souls/index.ts";
import { style } from "../../lib/terminal/index.ts";
import { withRunDb } from "./with_run_db.ts";

function findDeletedSoul(
  ...
  const match = deleted.find((s) => s.name.toLowerCase() === trimmed.toLowerCase());
  if (!match) return null;
  return getSoul(db, match.id);
}

export default defineCommand({
  meta: { name: "restore", description: "Restore an archived soul" },
  args: {
    name: {
      type: "positional",
      description: "Archived soul ID or name to restore",
      required: true,
    },
    as: {
      type: "string",
  ...
  async run({ args }) {
    ...
    await withRunDb((db) => {
      const soul = findDeletedSoul(db, soulArg);
      if (!soul) {
        console.error(
          style.boldRed("error".padStart(10)),
          ` Archived soul "${soulArg}" not found.`,
        );
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        const restored = restoreSoul(db, soul.id, newName);
        console.log(style.cyan("restored".padStart(10)), ` "${restored.name}"`);
      } catch (err) {
