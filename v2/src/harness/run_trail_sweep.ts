...
function buildTurn3(context: SweepContext): string {
  ...
  return [
    ...
    "   dismiss low-signal ones. Each loop needs a recommendedAction (ask, revisit,",
    "   remind, wait, leave) and optional earliestResurface (unix ms).",
    "   When a loop originates from a specific subsystem, tag it with sourceType",
    '   (e.g. "quest", "skill") and sourceId (the entity ID as string).',
    "",
